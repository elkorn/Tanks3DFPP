...
MTPixelToFrame MultiTexturedPS(MTVertexToPixel PSIn)
{
	...
	if(xEnableBlending)
	{
		float blendDistance = 0.99f;	// the distance to begin blending from
		float blendWidth = 0.005f;		// width of the blending border
		float blendFactor = clamp((PSIn.Depth-blendDistance)/blendWidth, 0, 1);
		float4 farColor = tex2D(TextureSampler0, PSIn.TextureCoords) * PSIn.TextureWeights.x;
